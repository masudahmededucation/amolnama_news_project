...
DROP TABLE IF EXISTS [debate].[coll_post_edit_history];
GO
DROP TABLE IF EXISTS [debate].[fact_post_moderation];
GO
DROP TABLE IF EXISTS [debate].[coll_vote];
GO
DROP TABLE IF EXISTS [debate].[coll_post];
GO
DROP TABLE IF EXISTS [debate].[coll_topic_participant];
GO
DROP TABLE IF EXISTS [debate].[coll_topic];
GO
DROP TABLE IF EXISTS [debate].[ref_vote_target_type];
GO
DROP TABLE IF EXISTS [debate].[ref_moderation_status];
GO
DROP TABLE IF EXISTS [debate].[ref_post_kind];
GO
DROP TABLE IF EXISTS [debate].[ref_topic_status];
GO
DROP TABLE IF EXISTS [debate].[ref_team_side];
GO

/* ================================================================
   TABLE: [debate].[ref_team_side]
   Blue (Pro / In Favor) vs Red (Against)
   ================================================================ */

CREATE TABLE [debate].[ref_team_side]
(
...
ALTER TABLE [debate].[ref_team_side]
...
SET IDENTITY_INSERT [debate].[ref_team_side] ON;
INSERT INTO [debate].[ref_team_side]
...
SET IDENTITY_INSERT [debate].[ref_team_side] OFF;
GO

/* ================================================================
   TABLE: [debate].[ref_topic_status]
   Debate lifecycle: draft → scheduled → live → paused → closed → archived
   ================================================================ */

CREATE TABLE [debate].[ref_topic_status]
(
...
ALTER TABLE [debate].[ref_topic_status]
...
SET IDENTITY_INSERT [debate].[ref_topic_status] ON;
INSERT INTO [debate].[ref_topic_status]
...
SET IDENTITY_INSERT [debate].[ref_topic_status] OFF;
GO

/* ================================================================
   TABLE: [debate].[ref_post_kind]
   Argument (new top-level point) vs Rebuttal (reply = opposition)
   ================================================================ */

CREATE TABLE [debate].[ref_post_kind]
(
...
ALTER TABLE [debate].[ref_post_kind]
...
SET IDENTITY_INSERT [debate].[ref_post_kind] ON;
INSERT INTO [debate].[ref_post_kind]
...
SET IDENTITY_INSERT [debate].[ref_post_kind] OFF;
GO

/* ================================================================
   TABLE: [debate].[ref_moderation_status]
   Moderation pipeline for post quality control
   ================================================================ */

CREATE TABLE [debate].[ref_moderation_status]
(
...
ALTER TABLE [debate].[ref_moderation_status]
...
SET IDENTITY_INSERT [debate].[ref_moderation_status] ON;
INSERT INTO [debate].[ref_moderation_status]
...
SET IDENTITY_INSERT [debate].[ref_moderation_status] OFF;
GO

/* ================================================================
   TABLE: [debate].[ref_vote_target_type]
   Vote can target a topic or a post
   ================================================================ */

CREATE TABLE [debate].[ref_vote_target_type]
(
...
ALTER TABLE [debate].[ref_vote_target_type]
...
SET IDENTITY_INSERT [debate].[ref_vote_target_type] ON;
INSERT INTO [debate].[ref_vote_target_type]
...
SET IDENTITY_INSERT [debate].[ref_vote_target_type] OFF;
GO
...
CREATE TABLE [debate].[coll_topic]
(
...
ALTER TABLE [debate].[coll_topic]
    ...
    REFERENCES [debate].[ref_topic_status] ([debate_ref_topic_status_id]);
GO
...
CREATE NONCLUSTERED INDEX [IX_debate_coll_topic_scheduled_start_at]
    ON [debate].[coll_topic] ([scheduled_start_at] ASC);
GO

CREATE NONCLUSTERED INDEX [IX_debate_coll_topic_link_topic_status_id]
    ON [debate].[coll_topic] ([link_topic_status_id], [is_active]);
GO

/* ================================================================
   TABLE: [debate].[coll_topic_participant]
   One user joins one topic on exactly one side
   ================================================================ */

CREATE TABLE [debate].[coll_topic_participant]
(
...
ALTER TABLE [debate].[coll_topic_participant]
...
ALTER TABLE [debate].[coll_topic_participant]
    ...
    REFERENCES [debate].[coll_topic] ([debate_coll_topic_id]);
GO
...
ALTER TABLE [debate].[coll_topic_participant]
    ...
    REFERENCES [debate].[ref_team_side] ([debate_ref_team_side_id]);
GO

CREATE UNIQUE NONCLUSTERED INDEX [UX_debate_coll_topic_participant_topic_user]
    ON [debate].[coll_topic_participant] ([link_topic_id], [link_user_profile_id]);
GO

CREATE NONCLUSTERED INDEX [IX_debate_coll_topic_participant_topic_side]
    ON [debate].[coll_topic_participant] ([link_topic_id], [link_team_side_id], [is_active], [is_banned]);
GO
...
ALTER TABLE [debate].[coll_post]
    ...
    REFERENCES [debate].[coll_topic_participant] ([debate_coll_topic_participant_id]);
GO
...
ALTER TABLE [debate].[coll_post]
    ...
    REFERENCES [debate].[ref_post_kind] ([debate_ref_post_kind_id]);
GO
...
ALTER TABLE [debate].[coll_post]
    ...
    REFERENCES [debate].[coll_post] ([debate_coll_post_id]);
GO
...
CREATE NONCLUSTERED INDEX [IX_debate_coll_post_topic_board_root]
    ON [debate].[coll_post] ([link_topic_id], [link_thread_board_side_id], [link_parent_post_id], [score] DESC, [posted_at] ASC)
...
CREATE NONCLUSTERED INDEX [IX_debate_coll_post_root_post]
    ON [debate].[coll_post] ([link_root_post_id], [post_reply_depth] ASC, [posted_at] ASC);
GO

/* Parent lookup */
CREATE NONCLUSTERED INDEX [IX_debate_coll_post_parent]
    ON [debate].[coll_post] ([link_parent_post_id], [posted_at] ASC);
GO

/* Author posts */
CREATE NONCLUSTERED INDEX [IX_debate_coll_post_author]
    ON [debate].[coll_post] ([link_author_user_profile_id], [posted_at] DESC);
GO

/* ================================================================
   TABLE: [debate].[fact_post_moderation]
   Separate audit trail — don't bury moderation inside the post row
   ================================================================ */

CREATE TABLE [debate].[fact_post_moderation]
(
...
ALTER TABLE [debate].[fact_post_moderation]
...
ALTER TABLE [debate].[fact_post_moderation]
    ...
    REFERENCES [debate].[ref_moderation_status] ([debate_ref_moderation_status_id]);
GO
...
CREATE NONCLUSTERED INDEX [IX_debate_fact_post_moderation_post]
    ON [debate].[fact_post_moderation] ([link_post_id], [moderated_at] DESC);
GO

/* ================================================================
   TABLE: [debate].[coll_vote]
   One vote per user per target — supports topics and posts
   ================================================================ */

CREATE TABLE [debate].[coll_vote]
(
...
ALTER TABLE [debate].[coll_vote]
    ...
    REFERENCES [debate].[ref_vote_target_type] ([debate_ref_vote_target_type_id]);
GO

/* One vote per user per target */
CREATE UNIQUE NONCLUSTERED INDEX [UX_debate_coll_vote_voter_target]
    ON [debate].[coll_vote] ([link_voter_user_profile_id], [link_vote_target_type_id], [target_row_id]);
GO

/* Aggregate votes for a target */
CREATE NONCLUSTERED INDEX [IX_debate_coll_vote_target]
    ON [debate].[coll_vote] ([link_vote_target_type_id], [target_row_id], [vote_value]);
GO

/* ================================================================
   TABLE: [debate].[coll_post_edit_history]
   Audit trail for edits — keep previous content
   ================================================================ */

CREATE TABLE [debate].[coll_post_edit_history]
(
...
ALTER TABLE [debate].[coll_post_edit_history]
...
CREATE NONCLUSTERED INDEX [IX_debate_coll_post_edit_history_post]
    ON [debate].[coll_post_edit_history] ([link_post_id], [edited_at] DESC);
GO
