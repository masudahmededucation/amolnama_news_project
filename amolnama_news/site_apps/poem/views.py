...
def poem_detail(request, poem_id):
    ...
    # Related poems (same category, exclude current)
    related = list(
        CollPoemEntry.objects
        .filter(link_poem_ref_poem_category_id=poem.link_poem_ref_poem_category_id)
        .exclude(poem_coll_poem_entry_id=poem_id)
