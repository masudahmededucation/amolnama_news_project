...
def lists_page(request):
    ...
    from .models import CollUserList
    ...
    user_lists = CollUserList.objects.filter(
        link_owner_user_profile_id=current_profile.user_profile_id, is_active=True,
    ).order_by('-created_at')

    list_items = []
    for user_list in user_lists:
        from .models import CollUserListMember
        member_count = CollUserListMember.objects.filter(
            link_list_id=user_list.social_coll_user_list_id, is_active=True,
        ).count()
        list_items.append({
            'list_id': user_list.social_coll_user_list_id,
